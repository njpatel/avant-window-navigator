...
glass_engine (cairo_t *cr, double x, gint width, gint height)
{
	...
	render_rect (cr, x, height/2, width, height/2);
	...
	render_rect (cr, x+1, height/2, width-2, height/5);
	cairo_set_source(cr, pat);
	cairo_fill(cr);
	cairo_pattern_destroy(pat);
...
render (cairo_t *cr, gint x_width, gint height)
{
	...
	render_rect (cr, x+1.5, (height/2)+1.5, width-2, (height/2)-2);
	//cairo_fill(cr);
	cairo_stroke(cr);
	...
	*/
	...
	render_rect (cr, x +0.5, (height/2)+0.5, width, (height/2));
	cairo_stroke(cr);
}
